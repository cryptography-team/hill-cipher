...
template <typename T> matrix<T>::~matrix() { erase(data, rows); }
...
template <typename T>
T **matrix<T>::create(const int &rows_, const int &cols_) const {
  T **data_ = new T *[rows_];
...
template <typename T> void matrix<T>::erase(T **data_, const int &rows_) {
  if (!data)
    return;
  for (int i = 0; i < rows_; i++)
    delete[] data_[i];
